...
app = Flask(__name__)
df=pd.read_csv(r'creditcard.csv', encoding='latin1')
X = df.iloc[:,:-1]
y = df.iloc[:,-1]
...
def preprocess():
    # file = os.listdir(app.config['upload_folder'])
    # path = os.path.join(app.config['upload_folder'], file[0])
    df = pd.read_csv(r'creditcard.csv', encoding= 'latin1')  # Specify the encoding here
    print(df.head())
    df.isnull().sum()
...
def model():
    ...
    if request.method == "POST":
        ...
        path = os.path.join(app.config['upload_folder'], file[0])
        df = pd.read_csv(r'creditcard.csv', encoding= 'latin1')
        df[['V1', 'V2', 'V3', 'V4', 'V5', 'V6']] = df[['V1', 'V2', 'V3', 'V4', 'V5', 'V6']].apply(pd.to_numeric, errors='coerce')
        df.fillna(method='ffill', inplace=True)
        df1 = df.sample(frac=0.3)
        # df = df.round(3)
